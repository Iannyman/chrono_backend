  ...
  async getSessionsDataLive(
    ...
    try {
      ...
      logger.error({
        rawResult: result.output.result,
        error: parseError instanceof Error ? parseError.message : String(parseError),
      }, 'Failed to parse sessions data live SP response as JSON');
      throw new Error('Invalid JSON response from sessions data live SP');
    }

    if (!response || typeof response !== 'object' || !Array.isArray(response.data)) {
      logger.error({ sqlResponse: response }, 'Sessions data live SP returned unexpected shape');
      throw new Error('Sessions data live SP response missing expected data array');
    }
